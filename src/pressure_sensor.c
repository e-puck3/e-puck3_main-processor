...
static lps22hd_data_t lps22hd_data;
...
static THD_FUNCTION(thdPressureSensor,arg) {
	...
	while(true){
		...
		lps22hd_read_converted_data(&lps22hd, &lps22hd_data);
...
void pressure_sensor_get_data(pressure_sensor_data_t* data){
	...
	data->pressure_raw 			= lps22hd_data.pressure_raw;
	data->pressure_hpa			= lps22hd_data.pressure_hpa;
	data->temperature_raw		= lps22hd_data.temperature_raw;
	data->temperature_deg_c		= lps22hd_data.temperature_deg_c;
}
