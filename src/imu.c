...
static lsm6dsox_data_t _lsm6dsox_data;
...
static THD_FUNCTION(thdIMU,arg) {
	...
	while(true){
		...
		lsm6dsox_read_converted_data(&lsm6dsox, &_lsm6dsox_data);
...
void imu_get_data(imu_data_t* data){
	...
	for(i = 0 ; i < IMU_NB_AXIS ; i++){
		data->rate_raw[i] 			= _lsm6dsox_data.rate_raw[i];
		data->rate_rad[i]			= _lsm6dsox_data.rate_rad[i];
		data->acceleration_raw[i]	= _lsm6dsox_data.acceleration_raw[i];
		data->acceleration_ms2[i]	= _lsm6dsox_data.acceleration_ms2[i];
	}
}
